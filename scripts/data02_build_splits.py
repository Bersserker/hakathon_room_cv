...
def require_columns(df: pd.DataFrame, required: set[str], source_path: Path) -> None:
    """Проверяет, что в таблице есть все обязательные колонки.

    Пояснение: страхует от запуска на CSV или parquet с неправильной схемой.
    """
...
def load_labeled_csv(path: Path, source_dataset: str) -> pd.DataFrame:
    """Загружает размеченный CSV, валидирует схему и добавляет имя источника.

    Пояснение: читает train/val таблицу и слегка приводит её к единому виду.
    """
    df = pd.read_csv(path)
    require_columns(df, REQUIRED_CSV_COLUMNS, path)
    df = df.copy()
    df["source_dataset"] = source_dataset
    df = df.sort_values(
        ["item_id", "image_id_ext", "image"], kind="stable"
...
def main() -> None:
    ...
    train_df_raw = load_labeled_csv(args.train_csv, "train_df")
    val_df_raw = load_labeled_csv(args.val_csv, "val_df")
